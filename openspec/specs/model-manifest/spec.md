...
## Purpose
TBD - created by archiving change fix-cicd-pipeline. Update Purpose after archive.
## Requirements
...
<!-- reconciled 2026-09-18 at triage: deploy.yml has never run (no self-hosted runner or production Environment exists); hosts deploy by pulling the committed generated files. -->
Deploying a manifest change SHALL apply the committed generated files: the rendered `litellm/config.yaml` on Prod and the rendered `models.generated.env` and download list on PEA, obtained by `git pull` of the same commit. PEA SHALL re-run `render-config.py` before `docker compose up`, so the host never runs a config that differs from the manifest.

#### Scenario: Manifest change rolls out by pull
- **WHEN** a `models.yaml` change is merged and each host pulls that commit and restarts the affected services
- **THEN** Prod serves the rendered LiteLLM config and PEA runs with the rendered `models.generated.env`
