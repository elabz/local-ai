# inference-metrics Specification

## Purpose
Every inference server on PEA exposes Prometheus metrics, every llama-server exposes llama.cpp's own `llamacpp:*` series (chat workers relay them at `/llama/metrics`), and PEA's Prometheus scrapes exactly the containers that are deployed.
## Requirements
