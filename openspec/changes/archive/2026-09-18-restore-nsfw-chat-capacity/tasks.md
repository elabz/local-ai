...
Verified live (read-only): `pea-gpu-4/5/6` all healthy, and `heartcode-chat-nsfw` has 3 deployments in Prod's LiteLLM config. The GPU 5 (`GPU-d8525241`) llama-server allocation is 6,366 MiB, under the 6.6 GiB ceiling. `scripts/check-placement.py` on PEA reports "live placement matches gpu-topology.json and LiteLLM routing" (the controller inventory was not compared; it needs sudo). Both spec requirements hold.

Dropped at triage: 2.4 (re-pin the SFW canary to GPU 6). Overtaken: the canary trials concluded (`evaluate-modern-chat-model-canaries`, 24/24), GPU 6 is back on its production layout (NSFW chat + DINO), and no canary container runs.
