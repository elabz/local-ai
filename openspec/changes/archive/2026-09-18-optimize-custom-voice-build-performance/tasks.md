...
Verified in code (read-only): resumable identity-bound state, live-service pause accounting and exhaustive-equivalent scoring are in `gpu-server/custom_voice/build_runtime.py`; the adaptive stop is `plateau.v1` (`validate_plateau_policy`), disabled by default. The focused build-runtime and worker tests pass (37/37). All five spec requirements hold as written.

Dropped at triage: 1.1 (standalone baseline benchmark; 9.3 already compared optimized against baseline) and 7.4 (batch-size benchmark; batching stays gated off until a Kokoro/Resemblyzer batch API is selected, per `BUILD-PERFORMANCE.md`). Re-propose only if batching is pursued.
