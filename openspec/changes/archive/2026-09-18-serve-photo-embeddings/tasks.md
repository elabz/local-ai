...
- [x] 3.2 Activate the `heartcode-embed-vision` entry in `litellm/config.yaml` (uncomment, set `api_base`/port) — active deployment → `192.168.70.144:8101/v1`, `mode: embedding`, rpm 40; `heartcode-embed` (BiQwen2.5) marked shelved. _Repo config done; prod LiteLLM restart (192.168.70.152) pending._
...
Verified live (read-only) against `embed-vision-1`: text and image both embed to 768-d in one space, `/health` reports the dimension, a mixed batch preserves order and index, a malformed image returns 400 without crashing, and nothing is stored. All spec requirements hold as written.

Dropped at triage: 1.1–1.4 (offline model eval; see `finalize-vision-embed-rollout` closure) and 4.2 (validation done at triage).
