...
All tasks were complete. Before archiving, the `gpu-rebalance` MODIFIED block was checked against the current main requirement "Embeddings co-located on chat GPUs": the base text is identical, so the delta only adds the canary exception and scenario and overwrites nothing newer. The canary-evaluation requirements stay as the procedure for future canaries.

Follow-up recorded (not fixed at triage, since it is a production config change): `litellm/config.base.yaml` still routes `heartcode-chat-gemma-3-12b-canary` (:18085) and `heartcode-chat-gemma4-luchador-nsfw-canary` (:18086), but no canary container runs on PEA, which breaks the file's own "only when actually loaded" rule.
