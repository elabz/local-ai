...
Verified (read-only): `GPU Server Build` has passed on every push since 2026-09-16 (last: d1bb71b); actions are on current majors with no deprecation annotations; `render-config.py --check` runs in CI. `deploy.yml` has **never run**: `gh api .../actions/runners` lists no runners and the repo has no `production` Environment. Both hosts deploy by `git pull` (Prod at 12d9da1, PEA at d1bb71b on 2026-09-18).

Dropped at triage: 3.1 and 3.2 (runner registration, Environment and secrets) and with them the entire `cd-deploy` delta, which described a pipeline that has never operated. The `model-manifest` "Deploy applies manifest-derived config" requirement was reconciled to the git-pull deploy that actually happens. `.github/workflows/deploy.yml` stays in the repo, dormant; if CD is still wanted, re-propose it as a new change that starts with registering the runner.
