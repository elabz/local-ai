...
Verified (read-only): `embed-vision-1` `/health` reports 768-d fp32; a mixed text/image/text batch returns three 768-d vectors in input order; an undecodable image returns 400 and the server keeps serving; `heartcode-embed-vision` (×2) and `heartcode-embed` (×2, `:8093`/`:8094`, both `/health` 200) are in Prod's LiteLLM config at 12d9da1. The proxy path relies on the 1.4 evidence (no key available at triage).

Dropped at triage: 2.1–2.3 (offline model eval — nomic-embed-vision-v1.5 has been the served model since rollout, and the downstream app already indexes its vectors); 3.2 (setup-doc refresh, folded into `project-health-improvements` section 4); 4.1 (superseded by this triage's validation). Spec reconciled to steady state, and the one-time rollback requirement removed. Re-propose the eval as its own change only if retrieval quality becomes a problem.
