...
- **Rollout finding:** `pea-gpu-2` was *recreated* at 17:30:07Z by something other than the rollout script, at the moment its GPU 2 neighbour `vision-embed-2` was being recreated. It came back correct (Stheno, `--cache-ram 1024`, 1792m). Inferred, not verified (the controller journal is root-only): the GPU failure controller saw a slot service missing and ran `compose up` for the whole slot with its own `gpu-uuid.override.json`, whose config hash differs from a plain compose invocation, so Compose recreated the chat container too. Future rollouts should recreate co-located slot services together, or check with root whether the controller's compose invocation matches the deploy one.
