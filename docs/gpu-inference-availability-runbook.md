...
2. Record the canary container restart count and start timestamp.
3. Copy only changed wrapper/runtime files, then recreate one SFW service.
4. Wait for `/live`, then model-aware `/health` and a fixed synthetic chat probe.
5. Run `scripts/sustained-chat-concurrency.py` for at least five watchdog
   intervals. Require successful completions and no restart-count increase.
6. Terminate only the canary's `llama-server` child. Require withdrawal,
